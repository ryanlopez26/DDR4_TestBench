...
mod backend {
    ...
    ///
    /// A no-op under `SIMULATION_MODE` - there's no real line to drive,
    /// and `lines()` would panic since `init()` is never called in that
    /// mode (see main.rs).
    ///
    /// # Panics
    /// Panics if called before a successful `init()` (outside simulation mode).
    ...
    ///
    /// A no-op under `SIMULATION_MODE` - see `set_fpga_reset`.
    ///
    /// # Panics
    /// Panics if called before a successful `init()` (outside simulation mode).
...
// defaults exactly - beam always low, controller always "calibrated",
// resets are harmless no-ops - so behaviour is identical to a hardware
...
mod backend {
    ...
    pub fn set_fpga_reset(_high: bool) -> Result<(), GpioError> {
        Ok(())
    }

    pub fn try_set_fpga_reset(_high: bool) -> Result<(), GpioError> {
        Ok(())
    }

    pub fn set_ddr4_controller_reset(_high: bool) -> Result<(), GpioError> {
        Ok(())
    }

    pub fn try_set_ddr4_controller_reset(_high: bool) -> Result<(), GpioError> {
        Ok(())
    }
